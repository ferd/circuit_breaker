...
prop_test() ->
    ?FORALL(Cmds, proper_fsm:commands(?MODEL),
        ?TRAPEXIT(
            begin
                {ok, Pid} = circuit_breaker:start_link(),
                {History, State, Result} = proper_fsm:run_commands(?MODEL, Cmds),
                unlink(Pid),
                gen_server:stop(Pid),
                ?WHENFAIL(io:format("History: ~p\nState: ~p\nResult: ~p\n",
                                    [History,State,Result]),
                          aggregate(lists:zip(proper_fsm:state_names(History),
                                              command_names(Cmds)),
                                    Result =:= ok))
            end)
    ).
...
postcondition(ok, _, _Data, {call, _, timeout, _}, {error, timeout}) ->
    true;
postcondition(ok, _, _Data, {call, _, err, _}, {exception, {exit,Err}}) ->
    not lists:member(Err, [ignore1, ignore2]);
postcondition(ok, _, _Data, {call, _, ignored_error, _}, {exception, {exit,Err}}) ->
...
%% v ADDED
next_state_data(ok, _To, Data=#data{errors=N, timeouts=M}, _Res, {call,_,success,_}) ->
    if N > 0 -> Data#data{errors=N-1};
       M > 0 -> Data#data{timeouts=M-1};
       N =:= 0, M =:= 0 -> Data#data{registered=true}
    end;
next_state_data(ok, _To, Data=#data{errors=N, timeouts=M}, _Res, {call,_,ignored_error,_}) ->
    if N > 0 -> Data#data{errors=N-1};
       M > 0 -> Data#data{timeouts=M-1};
       N =:= 0, M =:= 0 -> Data#data{registered=true}
    end;
